...
#                     idx[1] = np.where(np.all([(y-ymed>0), (x-xmed>0), np.arctan((y-ymed)/(x-xmed))>=np.pi/8, np.arctan((y-ymed)/(x-xmed))<=np.pi*3/8], axis=0))[0]
#                     idx[2] = np.where(np.all([(y-ymed>0), (x-xmed>0), np.arctan((y-ymed)/(x-xmed))>np.pi*3/8, np.arctan((y-ymed)/(x-xmed))<np.pi/2], axis=0))[0]
#                     idx[3] = np.where(np.all([(y-ymed>0), (x-xmed<0), np.arctan((y-ymed)/(x-xmed))>-np.pi/2, np.arctan((y-ymed)/(x-xmed))<-np.pi*3/8], axis=0))[0]
#                     idx[4] = np.where(np.all([(y-ymed>0), (x-xmed<0), np.arctan((y-ymed)/(x-xmed))>=-np.pi*3/8, np.arctan((y-ymed)/(x-xmed))<=-np.pi/8], axis=0))[0]
#                     idx[5] = np.where(np.all([(y-ymed>0), (x-xmed<0), np.arctan((y-ymed)/(x-xmed))>-np.pi/8, np.arctan((y-ymed)/(x-xmed))<0], axis=0))[0]
#                     idx[6] = np.where(np.all([(y-ymed<0), (x-xmed>0), np.arctan((y-ymed)/(x-xmed))>-np.pi/8], axis=0))[0]
#                     idx[7] = np.where(np.all([(y-ymed<0), (x-xmed>0), np.arctan((y-ymed)/(x-xmed))<=-np.pi/8, np.arctan((y-ymed)/(x-xmed))>=-np.pi*3/8], axis=0))[0]
#                     idx[8] = np.where(np.all([(y-ymed<0), (x-xmed>0), np.arctan((y-ymed)/(x-xmed))<-np.pi*3/8, np.arctan((y-ymed)/(x-xmed))>-np.pi/2], axis=0))[0]
#                     idx[9] = np.where(np.all([(y-ymed<0), (x-xmed<0), np.arctan((y-ymed)/(x-xmed))<np.pi/2, np.arctan((y-ymed)/(x-xmed))>np.pi*3/8], axis=0))[0]
#                     idx[10] = np.where(np.all([(y-ymed<0), (x-xmed<0), np.arctan((y-ymed)/(x-xmed))<=np.pi*3/8, np.arctan((y-ymed)/(x-xmed))>=np.pi/8], axis=0))[0]
#                     idx[11] = np.where(np.all([(y-ymed<0), (x-xmed<0), np.arctan((y-ymed)/(x-xmed))<np.pi/8], axis=0))[0]
#                     idx[12] = np.where(np.all([(y-ymed==0), (x-xmed>0)], axis=0))[0]
#                     idx[13] = np.where(np.all([(y-ymed<0), (x-xmed == 0)], axis=0))[0]
#                     idx[14] = np.where(np.all([(y-ymed==0), (x-xmed<0)], axis=0))[0]
#                     idx[15] = np.where(np.all([(y-ymed>0), (x-xmed==0)], axis=0))[0]

def masks_to_flow_directions(masks):
    """ convert masks to flow directions using diffusion from center pixel
    Center of masks where diffusion starts is defined to be the 
    closest pixel to the median of all pixels that is inside the 
    mask. Result of diffusion is converted into flows by computing
    the gradients of the diffusion density map. 
    ...
    mu = np.zeros((2, Ly, Lx), np.float64)
    mu_c = np.zeros((Ly, Lx), np.float64)
    mu_d = np.zeros((Ly, Lx), np.float64)
    nmask = masks.max()
    slices = scipy.ndimage.find_objects(masks)
    dia = utils.diameters(masks)[0]
    s2 = (.15 * dia)**2
    for i, si in enumerate(slices):
        if si is not None:
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                ...
                idx_tmp = [*idx[0], *idx[1], *idx[2]]
                mu_d[sr.start+y[idx_tmp]-1, sc.start+x[idx_tmp]-1] = 1.0

                idx[3] = np.where(np.all([(y-ymed<0), (x-xmed>0), np.arctan((y-ymed)/(x-xmed))<=-np.pi/8, np.arctan((y-ymed)/(x-xmed))>=-np.pi*3/8], axis=0))[0]
                idx_tmp = [*idx[3]]
                mu_d[sr.start+y[idx_tmp]-1, sc.start+x[idx_tmp]-1] = 2.0 
                ...
                idx_tmp = [*idx[4], *idx[5], *idx[6]]
                mu_d[sr.start+y[idx_tmp]-1, sc.start+x[idx_tmp]-1] = 3.0

                idx[7] = np.where(np.all([(y-ymed<0), (x-xmed<0), np.arctan((y-ymed)/(x-xmed))<=np.pi*3/8, np.arctan((y-ymed)/(x-xmed))>=np.pi/8], axis=0))[0]
                idx_tmp = [*idx[7]]
                mu_d[sr.start+y[idx_tmp]-1, sc.start+x[idx_tmp]-1] = 4.0 
                ...
                idx_tmp = [*idx[8], *idx[9], *idx[10]]
                mu_d[sr.start+y[idx_tmp]-1, sc.start+x[idx_tmp]-1] = 5.0

                idx[11] = np.where(np.all([(y-ymed>0), (x-xmed<0), np.arctan((y-ymed)/(x-xmed))>=-np.pi*3/8, np.arctan((y-ymed)/(x-xmed))<=-np.pi/8], axis=0))[0]
                idx_tmp = [*idx[11]]
                mu_d[sr.start+y[idx_tmp]-1, sc.start+x[idx_tmp]-1] = 6.0 
                ...
                idx_tmp = [*idx[12], *idx[13], *idx[14]]
                mu_d[sr.start+y[idx_tmp]-1, sc.start+x[idx_tmp]-1] = 7.0 

                idx[15] = np.where(np.all([(y-ymed>0), (x-xmed>0), np.arctan((y-ymed)/(x-xmed))>=np.pi/8, np.arctan((y-ymed)/(x-xmed))<=np.pi*3/8], axis=0))[0]
                idx_tmp = [*idx[15]]
                mu_d[sr.start+y[idx_tmp]-1, sc.start+x[idx_tmp]-1] = 8.0 
    mu /= (1e-20 + (mu**2).sum(axis=0)**0.5)
    return mu_d[np.newaxis,:,:]

def labels_to_flow_directions(labels, files=None, redo_flows=False):
    """ convert labels (list of masks or flows) to flow directions for training model 
    ...
    if labels[0].shape[0] == 1 or labels[0].ndim < 3 or redo_flows: # flows need to be recomputed
        ...
        if files is not None:
            for flow, file in zip(flows, files):
                file_name = os.path.splitext(file)[0]
                tifffile.imsave(file_name+'_flow_directions.tif', flow)
    else:
        dynamics_logger.info('flows precomputed')
        flows = [labels[n].astype(np.float32) for n in range(nimg)]
    return flows
...
@njit('(float32[:,:,:], float32[:,:,:], int32[:,:], int32)', nogil=True)
def steps2D_direction(p, dP, inds, niter):
    shape = p.shape[1:]
...
def follow_flows(dP, mask=None, niter=200, interp=False, use_gpu=True, device=None):
    ...
    if len(shape)>2:
        ...
        inds = np.array(np.nonzero(np.abs(dP[0])>1e-3)).astype(np.int32).T
        print(inds.ndim, inds.shape[0])
        if inds.ndim < 2 or inds.shape[0] < 5:
            print("=====0======")
            dynamics_logger.warning('WARNING: no mask pixels found')
            return p, None
        if not interp:
            p = steps2D_direction(p, dP.astype(int), inds, niter)
            print("=====1======")
        else:
            print("=====2======")
...
def compute_masks(dP, cellprob, p=None, niter=200, 
                   ...
                   flow_threshold=0.4, interp=False, do_3D=False, 
    ...
    if np.any(cp_mask): #mask at this point is a cell cluster binary map, not labels     
        # follow flows
        if p is None:
            p, inds = follow_flows(dP * cp_mask, niter=niter, interp=interp, 
