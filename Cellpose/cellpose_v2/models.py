...
class CellposeModel(UnetModel):
    ...
    def eval(self, x, batch_size=8, channels=None, channel_axis=None, 
        ...
        if isinstance(x, list) or x.squeeze().ndim==5:
            ...
            for i in iterator:
                ...
                dPs.append(dPi) 
                cellprobs.append(cellprobi)
            # return masks, flows, styles
            return dPs, cellprobs
            ...
            dP, cellprob, styles = self._run_cp(x, 
                                                          compute_masks=compute_masks,
                                                          normalize=normalize,
                                                          invert=invert,
                                                          rescale=rescale, 
    ...
    def _run_cp(self, x, compute_masks=True, normalize=True, invert=False,
        ...
        if do_3D:
            ...
            if resample:
                dP = np.zeros((8, nimg, shape[1], shape[2]), np.float32)
                cellprob = np.zeros((nimg, shape[1], shape[2]), np.float32)
            else:
                dP = np.zeros((8, nimg, int(shape[1]*rescale), int(shape[2]*rescale)), np.float32)
    ...
    def loss_fn(self, lbl, y):
        ...
        t1 = self._to_device(lbl[:,1], tensor_type="long")
        t2  = self._to_device(lbl[:,0]>.5, tensor_type="float")
        loss1 = self.criterion(y[:,0:8] , t1) 
        loss2 = self.criterion2(y[:,8] , t2)
        loss = loss1 + loss2
        return loss        
    ...
    def train(self, train_data, train_labels, train_files=None, 
        ...
        train_flows = dynamics.labels_to_flow_directions(train_labels, files=train_files)
        if run_test:
            test_flows = dynamics.labels_to_flow_directions(test_labels, files=test_files)
        else:
            test_flows = None
