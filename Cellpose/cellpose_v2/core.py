...
class UnetModel():
    ...
    def _to_device(self, x, tensor_type="float"):
...
#         slc = [slice(0, self.nclasses) for n in range(imgs.ndim)] # changed from imgs.shape[n]+1 for first slice size 
    ...
    def _train_step(self, x, lbl):
        X = self._to_device(x, tensor_type="float")
    ...
    def _train_net(self, train_data, train_labels, 
        ...
        if not osp.exists(osp.join(file_path, 'log.csv')):
            with open(osp.join(file_path, 'log.csv'), 'w') as f:
                f.write(','.join(['epoch', 'train/loss', 'valid/loss', 'elapsed_time']) + '\n')
        for iepoch in range(self.n_epochs):
            ...
            for ibatch in range(0, nimg_per_epoch, batch_size):
                ...
                imgi, lbl, scale = transforms.random_rotate_and_resize_directions(
                                        [train_data[i] for i in inds], Y=[train_labels[i][1:] for i in inds],
                                        rescale=rsc, scale_range=scale_range, unet=True)
            ...
            if test_data is not None:
                ...
                for ibatch in range(0, len(test_data), batch_size):
                    ...
                    imgi, lbl, scale = transforms.random_rotate_and_resize_directions([test_data[i] for i in inds], Y=[test_labels[i][1:] for i in inds], scale_range=0., rescale=rsc, unet=True)
                    if self.unet and lbl.shape[1] > 1 and rescale:
            ...
            print('Epoch %d, Time %4.1fs, Loss %2.4f, Loss Test %2.4f' % (iepoch, elapsed_time, lavg, lavgt/nsum))
            with open(osp.join(file_path, 'log.csv'), 'a') as f:
                log = [iepoch, lavg, lavgt/nsum, elapsed_time]
                log = map(str, log)
                f.write(','.join(log) + '\n')
            lavg, nsum = 0., 0
                            
            if save_path is not None:
                if iepoch==self.n_epochs-1 or (iepoch+1)%save_every==0:
